  ...
  !!  Double Do Loop to compute the A matrix
    ...
    DO i=1,npanels
       ...
       Do j=1,npanels
          ...
          ELSE
             vel(:,i,j)=panelinfluence(fieldpoint(:),center(:,j),area(j))
          END IF

          !Now get the influence of the panel 
          !on the mirrored points
          vp(:)=panelinfluence(fieldpointp(:),center(:,j),area(j))
          vpp(:)=panelinfluence(fieldpointpp(:),center(:,j),area(j))
          vppp(:)=panelinfluence(fieldpointppp(:),center(:,j),area(j))
...
!!---------------------------------------------------------------------
  ...
  !write(6,'(1D16.8)') vn
  !write(6,*)''
  !write(6,*) 'the total velocity on each panel'
...
ALLOCATE(velt(3,npanels))
     ...
     vtottest=-vinf
     Do j=1, npanels
        vtest(:,j) = panelinfluence(testpoint(:),center(:,j),area(j))

        !Now get the influence of the panel 
        !on the mirrored points
        vtp(:)=panelinfluence(testpointp(:),center(:,j),area(j))
        vtpp(:)=panelinfluence(testpointpp(:),center(:,j),area(j))
        vtppp(:)=panelinfluence(testpointppp(:),center(:,j),area(j))
        ...
        vtottest(:)=vtottest(:)+sigma(j)*velt(:,j)
