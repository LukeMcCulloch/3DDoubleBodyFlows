...
1 Write(6,*) 'Enter "1" if you would like to choose a test point '
Write(6,*) 'Enter "0" if you would like to end the program '
read(*,*) switch
...
read(*,*) testk
     ...
     Do j=1, npanels
        ...
        distance = sqrt(sum((testpoint(:)-center(:,j))**2.))

        !First get the singular points---------------------------------
        IF(abs(distance(1))<.0000001)THEN
             vtest(:,j)=-0.5*coordsys(:,3,i)
